...
export default function DashboardPage() {
  ...
  const downloadRangePDF = async () => {
    ...
    try {
      ...
      Object.keys(OUTLET_TARGETS).forEach(oid => { const nm = (OUTLET_NAMES as any)[oid] || oid; TGT_BY_NAME[nm] = parseFloat(OUTLET_TARGETS[oid] || "0") || 0; });
      const dayTgt = (d: any) => (Number(d.Target) || TGT_BY_NAME[d.Outlet] || 0);
      const pctCol = (pc: number) => pc >= 100 ? C.green : (pc >= 60 ? C.gold : C.red);
      const byO: Record<string, any> = {};
      data.forEach(d => { if (!byO[d.Outlet]) byO[d.Outlet] = { Outlet: d.Outlet, Days: 0, Shop: 0, Swiggy: 0, Zomato: 0, Total: 0, Target: 0 }; const b = byO[d.Outlet]; b.Days++; b.Shop += d.Shop; b.Swiggy += d.Swiggy; b.Zomato += d.Zomato; b.Total += d.Total; b.Target += dayTgt(d); });
      ...
      const html = `<div style="width:794px;background:${C.bg};font-family:'Segoe UI',Arial,sans-serif;color:${C.ink}">
        ...
        <table style="width:100%;border-collapse:collapse;background:${C.card};border:1px solid ${C.line};border-radius:12px;overflow:hidden;margin-bottom:14px">
            <thead><tr style="background:${C.ink}"><th style="padding:8px 10px;text-align:left;color:#FFF6E5;font-size:10px">OUTLET</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">DAYS</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">SHOP</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">SWIGGY</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">ZOMATO</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">TOTAL</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">TARGET</th><th style="padding:8px 10px;text-align:right;color:#FFF6E5;font-size:10px">%</th></tr></thead>
       <tbody>${sumRows}</tbody>
          </table>
         ${(() => {
